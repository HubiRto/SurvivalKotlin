...
class CreativeSub : SubCommand {
    override fun getName(): String {
        return "1"
    }

    override fun getDescription(): String {
        return "Zmienia tryb gry na kreatywny"
    }
        ...
        if (sender !is Player) {
            if (args.isEmpty()) {
                ...
                if (target == null) {
                    ...
                    sender.sendMessage(strToComp("<gray>Zmieniłeś tryb gracza <aqua>${target.name}</aqua> na <light_purple>CREATIVE"))
                }
            } else {
            ...
            if (args.isEmpty()) {
                ...
                if (target == null) {
                    ...
                    if (sender == target) {
                        sender.sendMessage(strToComp("<gray>Zamiast tego użyj komendy <red>/gm 1"))
                    } else {
                        target.gameMode = GameMode.CREATIVE
                        sender.sendMessage(strToComp("<gray>Zmieniłeś tryb gracza <aqua>${target.name}</aqua> na <light_purple>CREATIVE"))
                    }
